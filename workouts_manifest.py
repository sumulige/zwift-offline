...
def health_report(workouts_dir, filename):
    path = os.path.join(workouts_dir, filename)
    entries = {entry['name']: entry for entry in load_manifest(workouts_dir)}
    entry = entries.get(filename)
    report = {
        'directory': workouts_dir,
        'path': path,
        'manifest_path': manifest_file(workouts_dir),
